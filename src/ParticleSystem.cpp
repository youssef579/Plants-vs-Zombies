...
void ParticleSystem::spawnParticleEffect(ParticleEffect effectID, sf::Vector2f pos) {
  ...
  for (int i = 0; i < effectDef->emitters.size; i++) {
    ...
    for (int j = 0; j < e->spawnCount; j++) {
      ...
      p.sprite.setPosition({ pos.x + std::cos(angle) * radius, pos.y + std::sin(angle) * radius });
...
void ParticleSystem::update(float dt) {
  for (int i = 0; i < particles.size; i++) {
    ...
    p.sprite.setColor(sf::Color(
      (uint8_t)(curR * 255.0f),
      (uint8_t)(curG * 255.0f),
      (uint8_t)(curB * 255.0f),
      (uint8_t)(curAlpha * 255.0f)
    ));
...
void ParticleSystem::init() {
  ...
  powieText.startR = 1.0f; powieText.endR = 1.0f;
  powieText.startG = 1.0f; powieText.endG = 1.0f;
  powieText.startB = 1.0f; powieText.endB = 1.0f;
  ...
  smallClouds.startR = 1.0f; smallClouds.endR = 1.0f;
  smallClouds.startG = 0.7f; smallClouds.endG = 0.0f;
  smallClouds.startB = 0.0f; smallClouds.endB = 0.0f;
  ...
  bigClouds.startR = 1.0f; bigClouds.endR = 1.0f;
  bigClouds.startG = 0.9f; bigClouds.endG = 0.4f;
  bigClouds.startB = 0.0f; bigClouds.endB = 0.0f;
